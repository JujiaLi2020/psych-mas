...
def _detect_run_id_replica_hint(detect_error: str | None) -> bool:
    """POST/GET from Streamlit can hit different API replicas; disk job dirs are not shared."""
    if not detect_error:
        return False
    e = detect_error.lower()
    return (
        "each container has its own disk" in e
        or "multiple railway replicas" in e
        or "redis_url for shared" in e
    )
